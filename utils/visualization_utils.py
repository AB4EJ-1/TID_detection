...
# WDE
global firstBox, global_ymin, global_ymax, global_xmin, global_xmax
global_ymin = 0
global_xmin = 0
global_ymax = 0
global_xmax = 0
...
def draw_bounding_box_on_image_array(image,
  ...
  global firstBox, global_ymin, global_ymax, global_xmin, global_xmax   # WDE
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  print("in draw_bounding_box_on_image_array")
...
def draw_bounding_box_on_image(image,
 ...
 # print("DRAW BOX, normalized=",use_normalized_coordinates, "x1,x2,y1,y2:", xmin * im_width, xmax * im_width,
 #                                 ymin * im_height, ymax * im_height)
  if firstBox: # WDE section to find global (image) extents of detections
     print("first box")
     firstBox = False
     global_xmax = xmax * im_width
     global_xmin = xmin * im_width
     global_ymin = ymin * im_height
     global_ymax = ymax * im_height
  else:
     if xmax * im_width > global_xmax:
         global_xmax = xmax * im_width
     if xmin * im_width < global_xmin:
         global_xmin = xmin * im_width
     if ymax * im_height > global_ymax:
         global_ymax = ymax * im_height
     if ymin * im_height < global_ymin:
         global_ymin = ymin * im_height
  print("xmin:",global_xmin," xmax:",global_xmax," ymin:",global_ymin," ymax:",global_ymax)
...
def draw_bounding_boxes_on_image_array(image,
  ...
  """
  print("enter draw_bounding_boxes_on_image_array")
...
def draw_bounding_boxes_on_image(image,
  ...
  global firstBox, global_ymin, global_ymax, global_xmin, global_xmax   # WDE
  print("enter draw_bounding_boxes_on_image")
  ...
  for i in range(boxes_shape[0]):
    ...
    print("call draw_bounding_box_on_image")
...
def visualize_boxes_and_labels_on_image_array(
  ...
  # WDE - draw around all identified boxes
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB') # convert array to image
  draw = ImageDraw.Draw(image_pil)
  (left, right, top, bottom) = (global_xmin, global_xmax, global_ymin, global_ymax)
  thickness = 4
  # print("BOUNDING BOX - draw line:", left, right, top, bottom, thickness)
  TID_start = left /60 # TID start in hours
  TID_end = right / 60 # TID end in hours
  print("TID period start (hours past start time): ", "%.2f" % TID_start, " end: ", "%.2f" % TID_end)
  print("Period of detected TIDs: ","%.2f" % (TID_end - TID_start), " hours")
  df = open(output_path,'a+')
  df.write("%.2f" % TID_start + "," + "%.2f" % TID_end + "," + "%.2f" % (TID_end - TID_start) )
  df.close()
  draw.line([(left, top), (left, bottom), (right, bottom), (right, top),
               (left, top)],
              width=thickness,
              fill="white")
  np.copyto(image, np.array(image_pil)) # save it back as image array
  TID_time = TID_end - TID_start


  return image, TID_time
